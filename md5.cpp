...
DWordToByte( bits, (DWORD*)m_nCount, 8 );
...
DWordToByte( lpszMD5, (DWORD*)m_lMD5, nMD5Size );
...
std::string MD5(std::string in) {
	char out_char[32+1];
	char *in_buf=new char[in.length()+1];
	sprintf(in_buf,"%s",in.c_str());
	GetMD5(in_buf,in.length(),out_char);
	return std::string(out_char);
	}
